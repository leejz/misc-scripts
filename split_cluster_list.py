...
This script reads in a tab delimited file of clusters, and a list of number 
ranges for reads in each cluster.  The script will parse all reads based on the 
number ranges, and then output a file for each range.
...
--------------------------------------------------------------------------------      
usage:   split_cluster_list.py -i clusterlist.tab -f number1,number2,number3 -m mapping.list.txt
"""
...
if __name__ == '__main__':
    ...
    parser = ArgumentParser(usage = "split_cluster_list.py -i clusterlist.tab -f number1,number2,number3 -m mapping.list.txt (optional)",
                            description=__doc__, 
                            formatter_class=RawDescriptionHelpFormatter)
    parser.add_argument("-i", "--clusterlist_file", action="store", 
                         dest="clusterfilename",
                         help="tab-delimited cluster list file")
    parser.add_argument("-f", "--filter_list", action="store", 
                         dest="filterlist",
