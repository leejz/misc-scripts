#!/usr/bin/python
"""4/30/12 JZL
   This script reads a directory of information generated by CatchAll using the generate_Catchall_infile.py
   command and then extracts and summarizes all sample information into a excel tab delimited file for
   graphing.  Note, input data must be set up by generate_CatchAll_infile.py
...
if __name__ == '__main__':
    parser = OptionParser(usage = "usage: python collate_CatchAll_results.py -i input_directory -o output_file",
                  description='4/30/12 JZL collate_CatchAll_results.py.  This file relies on formatting \
                  generated by generate_CatchAll_infile.py')
            ...
            if os.path.isdir(raredir) == True:
                ...
                for subdirectory in CA_dir:
                    if os.path.isdir(raredir + "/" + subdirectory) == True:
                        ...
                        reader = csv.reader(open(filenamepath,'U'), dialect="excel")
    ...
    writer = csv.writer(open(outputfilename,'w'),dialect="excel-tab")
